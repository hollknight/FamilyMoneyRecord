...
func UpdateUserName(db *gorm.DB, username, name string) error {
	err := db.Model(&User{}).Where("username = ?", username).Update("name", name).Error
...
func UpdateAdvanceConsumption(db *gorm.DB, username string, advanceConsumption int) error {
	err := db.Model(&User{}).Where("username = ?", username).Update("advance_consumption", advanceConsumption).Error
...
func PlusUserReceiptSum(db *gorm.DB, user User, money int) error {
...
func PlusUserDisbursementSum(db *gorm.DB, user User, money int) error {
